...
(defn ui []
  [:footer.flex.flex-row.justify-between.w-full.mx-auto.py-4.max-w-screen-2xl
   ...
   [:div.flex.flex-row.justify-between
    [:a.block.mr-20px {:href "https://twitter.com/GloryGameNFT"} [:img.w-24px.h-24px {:src "/images/twitter-2.svg" :alt "link to twitter"}]]
    [:a.block.mr-20px {:href "https://opensea.io"} [:img.w-24px.h-24px {:src "/images/open-sea-2.svg" :alt "link to opensea"}]]
    [:a.block.mr-20px {:href "https://medium.com/glorygame"} [:img.w-24px.h-24px {:src "/images/medium-2.svg" :alt "link to medium"}]]
